...
void State:: get_info_for_view(int *turn_num, int rolls[5], int scores[2][2], bool *is_done, bool available_options[14], char **text_to_display, int *num_yahtzees)
{
	lock_funct();
...
void State::update_state_based_on_state(void *data, void(*determine_state)(void *data, int turn_number, int rolls[5], int &category, bool &is_done, bool is_selected[5], bool available_options[14], char *text_to_display))
{
	lock_funct();
...
void create_frame(vector<const char *> category_names, vector<vector<float>> &dice_coords, bool available_options[14], char *text_to_display, int turn_num, int num_yahtzees, int rolls[5], int scores[2][2])
{
	...
	if (text_to_display == NULL)
	{
		...
		if (turn_num < 2)
			show_text(coords_for_instructions[0], coords_for_instructions[1], "Select dice to keep by pressing dots. Press again to deselect.", white);
		else
	...
	show_text(coords[0], coords[1], "Done", red);
...
void Dice::update(int rolls[5], vector<vector<float>> &dice_coords)
{
	pos_of_dots.clear();
...
void onClick(int button, int state, int x, int y)
{
	...
	curr_state.update_state_based_on_state(&coords, [](void *data, int turn_num, int rolls[5], int &category, bool &is_done, bool is_selected[5], bool available_options[14], char *text_to_display)
	{
		int x = (*((int((*)[2]))(data)))[0];
		int y = (*((int((*)[2]))(data)))[1];
		...
		if (dist(pair < float, float > {x, y}, pair < float, float > {0.9 * Dimensions.window_width, 0.91 * Dimensions.window_height}) <= .06* Dimensions.window_height)
		{
			...
			if (curr_state.get_is_mouse_for_exiting())
				//glutExit();
				glutLeaveMainLoop();
			...
			{
				curr_state.is_view_blocked = true; //now not alloweed to draw the view until the state is updated
		...
		else if (curr_state.get_is_mouse_for_exiting())
			return;
...
void displayView()
{
	...
	if (curr_state.is_view_blocked)
	{
		...
		if (text != NULL)
			return; //there can be only one view display on CPU turn that ever is allowed to proceed
	...
	int turn_num;
	int num_yahtzees;
	curr_state.get_info_for_view(&turn_num, rolls, scores, NULL, available_options, &text_to_display, &num_yahtzees);
	//get the current state
	
	create_frame(category_names, dice_coords, available_options, text_to_display, turn_num, num_yahtzees, rolls, scores);


	if (text_to_display == NULL)
	{
		Dice dice = Dice();
		dice.update(rolls, dice_coords);
	}
